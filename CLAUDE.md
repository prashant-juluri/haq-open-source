...
- **Voice packs are prepared before onboarding begins.** `OnboardingStep.PreparingVoices` is the first step. On TTS ready, `OnboardingViewModel` calls `TTSManager.getMissingLanguages()` immediately — returning users advance at once. For first-launch users, `TTSManager.ensureAllVoicesDownloading()` calls `setLanguage()` for missing voices, triggering silent Google TTS background downloads. The polling loop (5s interval, 2-min max) drives progress. After `getMissingLanguages()` clears and the model is ready, `verifyAllVoicesSpeakable()` runs a `testSpeak()` gate before advancing to `LanguageSelect`.
- **`speak()` ERROR_OUTPUT (-4) recovery in onboarding: call `reinitialiseAndWait()` then `clearCachedVoice(lang)`.** After a -4 error the voice has disappeared from the engine's list. The correct recovery is `reinitialiseAndWait()` (which polls until the full voice list is stable) followed by `clearCachedVoice(languageCode)` so `findBestVoice()` re-scans the fresh list on the next attempt. The cached voice entry must be cleared after reinitialise — if it is not cleared, `findBestVoice()` returns the stale cached object which is no longer in the engine's voice list. The `speakOnboarding()` function in `OnboardingViewModel` does this: on -4 it sets the step to `PreparingVoices`, calls `reinitialiseAndWait()`, calls `clearCachedVoice(lang)`, sets the step back to `Introduction`, then retries. Up to 4 attempts total.
- **`checkLanguageSupport()` is unreliable — stub voices pass even without downloaded data.** Stub voices (names ending in `-language`) appear in the voice list but have no synthesis data and produce `ERROR_OUTPUT (-4)` when used. `checkLanguageSupport()` correctly filters stubs by name, but real-looking non-stub voice entries can also exist without downloaded data. **Always verify with `testSpeak()` during `PreparingVoices`.** If `testSpeak()` returns false for any language: set `_step = PreparingVoices` to keep the blocking screen visible, then poll `testSpeak()` sequentially per language every 5 seconds for up to 5 minutes until all pass. Do NOT advance to LanguageSelect until all 5 languages pass `testSpeak()`. `verifyAllVoicesSpeakable()` in `OnboardingViewModel` implements this pattern.
- **Never call `ACTION_INSTALL_TTS_DATA` — Google TTS downloads voice data silently on first use. Just poll `testSpeak()` and wait. Showing the installer screen is wrong UX and breaks the flow.**
...
- **`PreparingVoices` waits for both voices AND model.** `startVoiceReadinessPolling()` only advances to `LanguageSelect` when both `getMissingLanguages().isEmpty()` AND `GemmaManager.isModelReady(getApplication())` are true. The screen shows context-specific status: "Downloading AI model..." / "Preparing voices..." / "Downloading model and voices..." depending on what is missing.
- **`testSpeak()` is silent.** It passes `silent = true` to `speak()`, which builds a `Bundle` with `KEY_PARAM_VOLUME = "0"`. The user never hears the test utterance.
